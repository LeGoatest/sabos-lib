...
- Focus transfer between native chrome and web content must be predictable.
- Keyboard shortcuts must not conflict with screen readers or platform commands.
- Browser zoom restrictions must not prevent platform text scaling or application-level magnification.
- Deep links, refresh, offline, and restart behavior must preserve task context where practical.
- The embedded web view must expose the accessibility tree correctly on each supported platform.
- Platform-specific defects must be recorded in the browser and assistive-technology matrix.

Do not assume that accessible browser behavior automatically remains accessible inside every embedded web view.

## 6. Text, scaling, contrast, and user preferences

Software must support applicable platform settings for:
...
- Every operable control can receive and visibly indicate focus.
...
- Window restoration returns users to a meaningful location where practical.

## 8. Device features and permissions
...
## 9. Offline, installation, and updates
...
Every exported non-web document must select and document a format-specific accessibility baseline.
...
EPUB output must follow the applicable W3C EPUB specification and EPUB Accessibility standard selected by the project.

The export record must identify:
...
## 12. PDF and office-document output

For PDF or office formats, select the applicable format-specific accessibility standard or procurement requirement.

The generation process must not silently discard semantic source information. When a target format cannot preserve a required feature, the tool must warn the author and provide an accessible alternative or another export format.

## 13. Custom viewers and user agents
